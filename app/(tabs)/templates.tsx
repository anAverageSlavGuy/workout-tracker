...
export default function TemplatesScreen() {
  ...
  function handleEdit(t: WorkoutTemplate) {
    ...
    setName(t.name);
    setSelected(
      t.template_exercises?.map((te) => te.exercises!).filter(Boolean) ?? [],
    );
  }
  ...
  async function handleSaveEdit() {
    ...
    const currentExerciseIds = new Set(
      editingTemplate.template_exercises?.map((te) => te.exercise_id),
    );
    ...
    for (const exerciseId of currentExerciseIds) {
      if (!newExerciseIds.has(exerciseId)) {
        await deleteTemplateExercise.mutateAsync({
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.empty}>
            {isLoading ? "..." : "Nessuna scheda. Creane una."}
          </Text>
        }
        renderItem={({ item }) => (
      ...
      >
        <SafeAreaView style={styles.modal}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              const displayName = item.equipment_types
                ? `${item.name} (${item.equipment_types.name})`
                : item.name;
          ...
          <TouchableOpacity
            style={[
              styles.createBtn,
              (!name.trim() || createTemplate.isPending) &&
        ...
        <SafeAreaView style={styles.modal}>
          ...
          <TouchableOpacity
            style={[
              ...
              (!name.trim() ||
                ...
                deleteTemplateExercise.isPending ||
          ...
          >
            ...
            updateTemplate.isPending ? (
              ...
              <Text style={styles.createBtnText}>
                SALVA MODIFICHE ({selected.length})
              </Text>
            )}
          </TouchableOpacity>
        </SafeAreaView>
